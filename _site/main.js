...
(function () {
  ...
  var wrapper = document.getElementById('main_content'),
    ...
    startEffect = function () {
      ...
      options = Array.from(document.querySelectorAll('.pxxjs-option')).reduce((obj, e) => {
    ...
    externalPreview = function (source) {
      ...
      source =
        '<html><head><meta charset="utf-8"><link rel="shortcut icon" type="image/png" href="http://localhost:4000/pxxjs/favicon.png"><title>pxxjs | Preview</title><style>' +
        externalStyle +
    ...
    format = debounce(function () {
      ...
      if (!workerFormat) {
        workerFormat = new Worker('/pxxjs/worker/format.js');
    ...
    decode = debounce(function () {
      ...
      temp = temp.replace(/\/\*(?!\s*@pxxjs)[\s\S]*?\*\/|^[\s\t]*\/\/.*/gm, '');
      if (temp === '') return;
      ...
      if (!workerDecode) {
        workerDecode = new Worker('/pxxjs/worker/decode.js');
